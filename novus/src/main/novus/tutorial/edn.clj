...
(comment
  " Intro
    ...
    - EDN is a subset of the syntax used by Clojure. Reading data defined by
      EDN is safer than that defined by the full Clojure syntax, especially from untrusted
      sources. EDN is restricted to data, no code. It is similar in intent to JSON. Though
      it is more commonly used in Clojure, there are implementations of EDN for many other languages.
    - The main benefit of EDN over JSON and YAML is that it is extensible.
      We will see how it is extended later on.")
...
(clojure.edn/read-string
    {:readers {'novus.schema/MenuItem map->MenuItem}}
    "#novus.schema/MenuItem {:name \"eggs-benedict\" :rating 10}")
; -> #novus.schema.MenuItem {:name "eggs-benedict", :rating 10}
